...
- **Level-Aware Tasks**: A task whose feature the game has not unlocked yet is held back, shown on the status screen as `Locked (<level>)`, and starts on its own once the character reaches that level.
